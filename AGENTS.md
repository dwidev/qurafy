...
Use `npm install` to install dependencies. Use `npm run dev` to start the local Next.js server, `npm run build` to produce a production build, and `npm run start` to serve that build. Run `npm run lint` before opening a PR; this uses the root `eslint.config.mjs`. For schema changes, run `npm run db:generate` to create migration files and `npm run db:migrate` to apply them.
...
## Testing Guidelines
There is currently no committed automated test runner. Until one is added, treat `npm run lint` and a successful `npm run build` as the minimum validation for every change. When adding tests, place them next to the feature they cover and use `*.test.ts` or `*.test.tsx` naming so they are easy to discover later.
