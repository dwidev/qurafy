...
import { createMemorizeGoal, deleteMemorizeGoal, updateMemorizeGoal } from "@/features/memorize/server/memorize-data";
import type { CreateMemorizeGoalPayload, DeleteMemorizeGoalPayload, UpdateMemorizeGoalPayload } from "@/features/memorize/types";
...
export async function PATCH(request: NextRequest) {
  ...
  let payload: UpdateMemorizeGoalPayload;

  try {
    payload = (await request.json()) as UpdateMemorizeGoalPayload;
  } catch {
    return jsonError("Invalid request body.", 400);
  }

  try {
    const result = await updateMemorizeGoal(session.user.id, payload);
    revalidateTag(`dashboard-view-data:${session.user.id}`, "max");
    return NextResponse.json(result);
  } catch (error) {
    if (error instanceof Error) {
    ...
    return jsonError("Failed to update memorize goal.", 500);
  }
}
